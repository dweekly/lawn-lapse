...
async function checkStatus() {
  ...
  try {
    ...
    if (crontab.includes("capture-and-timelapse.js")) {
      ...
      try {
        ...
        const lastRun = lines
          .reverse()
          .find((line) => line.includes("Daily update complete"));
        if (lastRun) {
          const match = lastRun.match(/\[([\d\-T:.Z]+)\]/);
          if (match) {
            const lastRunDate = new Date(match[1]);
            const hoursAgo = Math.floor(
              (Date.now() - lastRunDate) / (1000 * 60 * 60),
            );
            console.log(
              `  Last run: ${lastRunDate.toLocaleString()} (${hoursAgo} hours ago)`,
            );
          }
        }
      } catch {
      ...
      console.log(
        "  Run: ./setup-daily-cron.sh to set up automatic daily capture",
      );
    }
  } catch {
  ...
  if (snapshotCount > 0) {
    ...
    console.log(
      '  💡 Tip: Run "npm run capture" to update snapshots and create video',
    );
  } else {
